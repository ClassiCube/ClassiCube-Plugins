...
namespace PluginObjExport {
	...
	public sealed class ObjExporterCommand : Command {
		...
		public override void Execute(string[] args) {
			if (args.Length == 1) {
				game.Chat.Add("&cFilename required.");
				return;
			}
			...
			using (FileStream fs = File.Create(path)) {
				...
				obj.all = args.Length > 2 && args[2].ToUpper() == "ALL";
				if (obj.all) {
					game.Chat.Add("&cExporting all faces - slow!");
				}
				
				obj.mirror = args.Length <= 3 || args[3].ToUpper() != "NO";
				if (!obj.mirror) {
	...
	public sealed class ObjExporter : IMapFormatExporter {
		...
		void DumpVertices() {
			...
			{
				++i; int block = (blocks[i] | (blocks2[i] << 8)) & mask;
				...
				// minx
				if (x == 0 || all || !IsFaceHidden(block, (blocks[i - oneX] | (blocks2[i - oneX] << 8)) & mask, Side.Left)) {
					w.WriteLine("v " + min.X + " " + min.Y + " " + min.Z);
					w.WriteLine("v " + min.X + " " + max.Y + " " + min.Z);
					w.WriteLine("v " + min.X + " " + max.Y + " " + max.Z);
					w.WriteLine("v " + min.X + " " + min.Y + " " + max.Z);
				...
				// maxx
				if (x == maxX || all || !IsFaceHidden(block, (blocks[i + oneX] | (blocks2[i + oneX] << 8)) & mask, Side.Right)) {
					w.WriteLine("v " + max.X + " " + min.Y + " " + min.Z);
					w.WriteLine("v " + max.X + " " + max.Y + " " + min.Z);
					w.WriteLine("v " + max.X + " " + max.Y + " " + max.Z);
					w.WriteLine("v " + max.X + " " + min.Y + " " + max.Z);
				...
				// minz
				if (z == 0 || all || !IsFaceHidden(block, (blocks[i - oneZ] | (blocks2[i - oneZ] << 8)) & mask, Side.Front)) {
					w.WriteLine("v " + min.X + " " + min.Y + " " + min.Z);
					w.WriteLine("v " + min.X + " " + max.Y + " " + min.Z);
					w.WriteLine("v " + max.X + " " + max.Y + " " + min.Z);
					w.WriteLine("v " + max.X + " " + min.Y + " " + min.Z);
				...
				// maxz
				if (z == maxZ || all || !IsFaceHidden(block, (blocks[i + oneZ] | (blocks2[i + oneZ] << 8)) & mask, Side.Back)) {
					w.WriteLine("v " + min.X + " " + min.Y + " " + max.Z);
					w.WriteLine("v " + min.X + " " + max.Y + " " + max.Z);
					w.WriteLine("v " + max.X + " " + max.Y + " " + max.Z);
					w.WriteLine("v " + max.X + " " + min.Y + " " + max.Z);
				...
				// miny
				if (y == 0 || all || !IsFaceHidden(block, (blocks[i - oneY] | (blocks2[i - oneY] << 8)) & mask, Side.Bottom)) {
					w.WriteLine("v " + min.X + " " + min.Y + " " + min.Z);
					w.WriteLine("v " + min.X + " " + min.Y + " " + max.Z);
					w.WriteLine("v " + max.X + " " + min.Y + " " + max.Z);
					w.WriteLine("v " + max.X + " " + min.Y + " " + min.Z);
				...
				// maxy
				if (y == maxY || all || !IsFaceHidden(block, (blocks[i + oneY] | (blocks2[i + oneY] << 8)) & mask, Side.Top)) {
					w.WriteLine("v " + min.X + " " + max.Y + " " + min.Z);
					w.WriteLine("v " + min.X + " " + max.Y + " " + max.Z);
					w.WriteLine("v " + max.X + " " + max.Y + " " + max.Z);
					w.WriteLine("v " + max.X + " " + max.Y + " " + min.Z);
