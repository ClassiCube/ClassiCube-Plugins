            ...
            for (int x = xStart; x < xStart + xWidth; ++x) { //from 0 to the width + height of the map
                for (int z = zStart; z < zStart + zLength; ++z) { //from 0 to the length of the map
                    ...
                    while (y > 0 &&
                           ...
                           zD >= 0 &&
                           zD < length &&
                           !(BlockInfo.BlocksLight[map.GetBlock(xD, y, zD)] ||
                             BlockInfo.BlocksLight[map.GetBlock(xD, ySafe, zD)]) &&
                           
                           !(BlockInfo.BlocksLight[map.GetBlock(xSafe, y, zD)] || BlockInfo.BlocksLight[map.GetBlock(xD, y, zSafe)]) &&
                           !(BlockInfo.BlocksLight[map.GetBlock(xSafe, ySafe, zD)] || BlockInfo.BlocksLight[map.GetBlock(xD, ySafe, zSafe)])
                          ) {
